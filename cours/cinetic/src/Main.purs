...
import Concur.Core.FRP (Signal, dyn, loopS, loopW, display, step)
...
incrementTicker :: Model -> Signal HTML Model
incrementTicker init = loopW init $ \m -> do
  liftAff $ delay $ Milliseconds 100.0
  pure m {tick = m.tick + 10}

incrementTicker' :: Model -> Signal HTML Model
incrementTicker' init = loopW init $ \m -> do
  liftAff $ delay $ Milliseconds 50.0
  pure m {tick' = m.tick' + 12}

evolveTicker :: Model -> Signal HTML Model
evolveTicker init = loopW init $ \m -> do
  liftAff $ delay $ Milliseconds 50.0
  pure m {shapes = evolve m.shapes}
                ...
                (\mouse ->
                   let a'0 /\ b'0 = pageX mouse /\ pageY mouse
                     ...
                     in model { lastReferencePosition = a'0 /\ b'0
                              , lastDependantPosition = (x+model.tick) /\ y
                              }
                )
                (fromEvent ev)
        )  <$> P.onMouseMove
...
modelWidget :: Model -> Signal HTML Model
modelWidget m = loopS m $ \model' -> do
   m' <- incrementTicker model'
   m'' <- incrementTicker' m'
...
{-  loopW model'{tick = n} \model -> do
     newModel <- S.svg
              [ S.width "500"
              , S.height "500"
              , (\ev ->
                  maybe model{shapes = sh}
                        (\mouse ->
                           let a'0 /\ b'0 = pageX mouse /\ pageY mouse
                               p0 /\ q0 = model.lastDependantPosition
                               a0 /\ b0 = model.lastReferencePosition
