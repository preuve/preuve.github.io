...
data Shape
  ...
  -- | A circular arc consisting of the numbers center-x, center-y, start angle, end angle , radius and spin
  | Arc Canvas.Arc
  -- | A composite shape
...
circle x y = arc x y 0.0 (pi * 2.0) true

-- | Create a circular arc from the left, top, start angle, end angle and
-- | radius parameters.
arc :: Number -> Number -> Number -> Number -> Boolean -> Number -> Shape
arc x y start end useCounterClockwise radius = Arc { x, y, start, end, radius, useCounterClockwise }
