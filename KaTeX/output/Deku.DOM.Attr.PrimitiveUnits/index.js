// Generated by purs version 0.15.9
import * as Deku_Attribute from "../Deku.Attribute/index.js";
var PrimitiveUnits = /* #__PURE__ */ (function () {
    function PrimitiveUnits() {

    };
    PrimitiveUnits.value = new PrimitiveUnits();
    return PrimitiveUnits;
})();
var attrPrimitiveUnitsUnit = {
    attr: function (v) {
        return function (v1) {
            return Deku_Attribute.unsafeAttribute({
                key: "primitiveUnits",
                value: Deku_Attribute["unset$prime"]
            });
        };
    }
};
var attrFilter_PrimitiveUnits = {
    attr: function (v) {
        return function (value) {
            return Deku_Attribute.unsafeAttribute({
                key: "primitiveUnits",
                value: Deku_Attribute["prop$prime"](value)
            });
        };
    }
};
export {
    PrimitiveUnits,
    attrFilter_PrimitiveUnits,
    attrPrimitiveUnitsUnit
};
