...
import Data.Geometry.Plane (circle, length, point, rightangle, segment, vector, scale, (<+|), rename)
import Concur.VDom.GeometryRender(class Render, render', defaultContext)
import Data.Array (replicate, sort, (:), catMaybes, all, (..), slice, zipWith)
...
pick xs r = xs !! (r.val `mod` (Array.length xs -1))
      ...
      then []
        ...
        put $ D.div [P.attr "style" "display: grid; grid-template-columns: 1fr 5fr ;"]
          ...
          , D.div' $ fromIncremental $ do
            nl
            t " Sur la figure (qui n'est pas à l'échelle), les droites "
            m "(BC)"
            t " et "
            m "(MN)"
            t " sont parallèles. On donne "
            let rab = rand r0
                ab = (_ + 3) $ rab.val `mod` 10
                rbm = rand rab
                bm = (_ + 3) $ rbm.val `mod` 10
                rac = rand rbm
                ac = (_ + 3) $ rac.val `mod` 10
