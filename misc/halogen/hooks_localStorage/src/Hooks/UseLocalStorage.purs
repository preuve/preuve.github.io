...
newtype UseLocalStorage a hooks =
  UseLocalStorage (UseEffect (UseInitializer (UseState (E a) hooks)))
...
useLocalStorage
  ...
  => Eq a
  => StorageInterface a
  -> Hook m (UseLocalStorage a) (E a /\ ((E a -> E a) -> HookM m Unit))
useLocalStorage { key, defaultValue, toJson, fromJson } = Hooks.wrap Hooks.do
  ...
  useWriteStorage deps = Hooks.captures deps Hooks.useTickEffect do
    liftEffect do
      ...
      for_ deps.value \v -> setItem deps.key (toJson v) storage
    pure Nothing
