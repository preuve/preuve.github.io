...
randFraction = \ r -> 
  let r0 = rand r
      ...
      prime = \ ix -> fromMaybe 1 $ primes !! ix 
...
body st = dyn $ do
  ...
  display $ D.div' $ if not newState.enabled 
      then []
        ...
        m $ "\\left\\{\\begin{array}{l}x_1+x_2=" 
                            <> (show $ f1.fraction + f2.fraction) 
                            <> "\\\\ x_1x_2 = " 
                            <> (show $ f1.fraction * f2.fraction)
                            <> "\\end{array}\\right."
        nl
        t "Donner l'ensemble des couples "
        m "(x_1, x_2)"
        t " possibles:"
        nl
        nl
        ...
        t " Soit "
        m "f"
        t " la fonction polynôme du second degré définie par "
        ...
        let b2 = f4.fraction
        let c2 = f5.fraction
        m $ "f(x)=" <> showTrinom a2 b2 c2
        t " pour tout "
        m "x\\in\\mathbb{R}"
        t ". "
        nl
        t "Donner la forme canonique de "
        m "f"
        t " :"
        nl
        nl
        ...
        m $ showTrinom (a3' / lift g3) (b3' / lift g3) (c3' / lift g3)
        t "."
        nl
        t "En déduire le nombre de solutions réelles de l'équation "
        equation $ (if a3 == a3' / lift g3
                    then showTrinom (a3 / lift (-2))  (b3 / lift (-2))  (c3 / lift (-2))
                    else showTrinom a3 b3 c3) <> "=0"
        t "On donnera la valeur du discriminant "
        t "ainsi que le nombre de solutions."
        nl
        t "Même s'il y a des solutions, on ne demande pas de calculer leur valeur:"
        nl
        nl
        nl
        ...
        let a5 = f12.fraction
        let b5 = f13.fraction
        m $ "f(x)=" <> showTrinom a5 b5 zero <> "+c"
        t " où "
        m "c"
        t " est un nombre réel."
        nl
        t "Déterminer la valeur de "
        m "c"
        t " pour laquelle "
        m "f"
        t " admet une unique racine (on dit aussi racine \"double\") "
        m "x_0"
        t ", puis calculer "
        m "x_0"
        t ":"
...
header = D.div' $ fromIncremental $ do
  ...
  put $ D.ul []
      ...
      , D.li [] (fromIncremental $ do
           ...
           get)
      , D.li [] [D.text "toute valeur numérique à donner sous forme entière ou rationnelle irréductible"]
