  ...
  var scene = function (mouse) {
      return function (v) {
          return function (v1) {
              var scaleY = v.y / v1.y;
              var scaleX = v.x / v1.x;
              var renderSegment = function (bs) {
                  return function (v2) {
                      var cursor = Graphics_Drawing.scale(scaleX)(scaleY)(Graphics_Drawing.outlined(Data_Semigroup.append(Graphics_Drawing.semigroupOutlineStyle)(Graphics_Drawing.outlineColor(Color.rgb(0)(0)(0)))(Graphics_Drawing.lineWidth(1.5)))(Graphics_Drawing.path(Data_Foldable.foldableArray)([ v2.from, v2.to ])));
                      var $9 = Data_Set.isEmpty(bs);
                      if ($9) {
              ...
              var coord = (function () {
                  var fromCoord = Data_Maybe.maybe(defaultSegment)(function (v2) {
                      return {
                          from: {
                              x: Data_Int.toNumber(v2.x) / scaleX,
                              y: Data_Int.toNumber(v2.y) / scaleY
                          },
                          to: defaultCoord
                      };
                  });
                  return Data_Functor.map(FRP_Behavior.functorABehavior(FRP_Event.functorEvent))(fromCoord)(FRP_Behavior_Mouse.position(mouse));
              })();
              var segment = (function () {
                  var toSegment = function (c) {
                      return function (s) {
                          return {
                              from: s.to,
                              to: c.from
                  ...
                  return FRP_Behavior.fixB(defaultSegment)(function (seg) {
                      return Control_Apply.apply(FRP_Behavior.applyABehavior(FRP_Event.functorEvent))(Data_Functor.map(FRP_Behavior.functorABehavior(FRP_Event.functorEvent))(toSegment)(coord))(seg);
                  });
              })();
              return Control_Apply.apply(FRP_Behavior.applyABehavior(FRP_Event.functorEvent))(Data_Functor.map(FRP_Behavior.functorABehavior(FRP_Event.functorEvent))(renderSegment)(FRP_Behavior_Mouse.buttons(mouse)))(segment);
          };
      };
  };
  var main = function __do() {
      var win = Web_HTML.window();
      var wh = Web_HTML_Window.innerHeight(win)();
      var ww = Web_HTML_Window.innerWidth(win)();
      ...
      if (mcanvas instanceof Data_Maybe.Just) {
          ...
          var cw = Graphics_Canvas.getCanvasWidth(mcanvas.value0)();
          var ch = Graphics_Canvas.getCanvasHeight(mcanvas.value0)();
          var mouse = FRP_Event_Mouse.getMouse();
          FRP_Behavior.animate(scene(mouse)({
              x: cw,
              y: ch
          })({
              x: Data_Int.toNumber(ww),
              y: Data_Int.toNumber(wh)
          }))(Graphics_Drawing.render(ctx))();
          return Data_Unit.unit;
      };
      throw new Error("Failed pattern match at Main (line 66, column 3 - line 76, column 18): " + [ mcanvas.constructor.name ]);
  };
  exports["defaultCoord"] = defaultCoord;
  exports["defaultSegment"] = defaultSegment;
  exports["scene"] = scene;
  exports["main"] = main;
})(PS);
PS["Main"].main();